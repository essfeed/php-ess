<?php
	...
	$essFeed->setRights( 	'Copyright (c) ' . date( 'Y' ) . ', ESS Generator' );
		...
		// --- Define the mandatory event's elements
		...
		// ----------- Add personalized category, without ID - Useful to create several specific event that belong to the same category.
		$newEvent->addCategory( 'competition', 	array('name'=> 'FunnySoccer' ) );
		
		
		
		// ====== DATES =========
		// ----------- add a repetitive event that appends 3 times every week at the same time then the first one and for 2 hours.
		...
		// ----------- add a repetitive event that appends every last sunday of every month for two years stating at the same time then the first one and for 2 hours.   
		...
		// ----------- add two simple date that appends only two times: 10/25/2013 and 11/25/2013 at 3:30pm (PST - Pacific Standard Time) for 3 hours.
		...
		// ----------- add a repetitive event that appends on monday, tuesday and thirsday every three weeks for six months starting at the same time then the first one (8am PST) and for 8 hours.
		...
		$newEvent->addPrice( 'standalone', 	null,	'free', null,null,null,null, array('name'=> 'Free Entrance', 'value'=> '0'));
		
		// ----------- Define a free entrance but restricted with a invitation 
		$newEvent->addPrice( 'standalone', 	null,	'invitation', null,null,null,null, array('name'=> 'Invitation required - please contact the organizer', 'value'=> '0'));
		
		// ----------- Define a specific VIP access for $10.
		$newEvent->addPrice( 'standalone', 	null,	'fixed',null,null,null,null, array('name'=> 'Entrance with VIP access', 'value'=> '10', 'currency'=> 'USD'));
		
		// ----------- Define a billing payment (required or made) every 28th of every month during 12 months with a payment URL.
		$newEvent->addPrice( 'recurrent', 	'month','fixed',12,28,"number",null, array('name'=> 'Monthly subscription - payment every 28th (for one year)', 'value'=> '20', 'currency'=> 'USD', 'start'=> '2013-10-25T23:59:00Z', 'uri'=>'http://payment.com/api'));
		
		// ----------- Define a pre-selling ticket available 1 month before the event (the date of the event have to be defined in Object->addDate(...) ) and for 1 week of selling availability
		$newEvent->addPrice( 'standalone', 	null, 'fixed',null,null,null,null, array('name'=> 'Tickets available 1 month before the event during 2 weeks.', 'value'=> '20', 'currency'=> 'USD', 'start'=> '2013-09-25T23:59:00Z', 'duration' => 3600*24*15, 'uri'=>'http://payment.com/api/script.do' ) );
		...
		// ---------- Define who is performing during the event (actors, players, singers, talker...)
		$newEvent->addPeople( 'performer', array('name' => 'The main player', 'firstname' => 'Christiano','lastname' => 'Ronaldo' ) );
		// ---------- Define requirements for the event: the stadium can contains only 2000 persones, the minimum age is 16yo and it's prohibited to smoke! 
		...
		// ====== MEDIA ============
		// ---------- add several media file to display and represent your event. The quality and the number of images are very important for events search engine and event's attendees.
		...
		// ====== RELATIONS ============
		// ---------- add several events in relation with current event. If you don't have the ID the event, you can generate it from the URL of the final event web page.
		...
		// ======================================================================
	  	...
	  	// <feed>
	  	// All the information defined inside the <feed> element must be applicable to every sub-elements:
	  	// All the dates defines muste be applicable to every prices defines and to every images defines...
	  	$newEvent = $essFeed->newEventFeed( array( 'title'=> 'Madonna Concert', 'published'=> FeedWriter::getISODate(), 'access'=> 'PUBLIC', 'description' => "This is the description of the Madonna concert.", 'tags'=> array( 'music', 'pop', '80s', 'Madonna', 'concert' )));
			...
			$newEvent->addPlace( 	'fixed', 										 array('name'=> 'Stadium', 'latitude'=> '40.71675', 'longitude' => '-74.00674', 'address' => 'Ave of Americas, 871', 'city' => 'New York', 'zip' => '10001', 'state' => 'New York', 'state_code' => 'NY', 'country' => 'United States of America', 'country_code' => 'US' ) );
			$newEvent->addPrice(	'standalone', null, 'fixed',null,null,null,null, array('name'=> 'Entrance with VIP access', 'value'=> '90', 'currency'=> 'USD', 'uri'=> 'http://madonna.com/payment/api'));
