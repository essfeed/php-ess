<?php
	...
	$essFeed->setRights( 	'Copyright (c) ' . date( 'Y' ) . ', ESS Generator' );
		...
		// --- Define mandatory event's elements
			...
			// --- PRICES
			$newEvent->addPrices(		'standalone', 	null,		array('name'=> 'Free Entrance', 'value'=> '0'));
			$newEvent->addPrices(		'standalone', 	null,		array('name'=> 'Entrance with VIP access', 'value'=> '10', 'currency'=> 'USD'));
			$newEvent->addPrices(		'recurent', 	'month',	array('name'=> 'Monthly subscription', 'value'=> '20', 'currency'=> 'USD', 'start'=> '2013-10-25T23:59:00Z', 'uri'=>'http://payment.com/api'));
	...
	// OR Get ESS Feed Data to create a file
	/*
	$fp = fopen( 'feed.ess', 'w' );
	fwrite( $fp, $essFeed->getFeedData() );
	fclose( $fp );
	*/